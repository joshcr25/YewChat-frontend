...
impl Component for Chat {
                    ...
                    <div class="w-full grow overflow-auto border-b-2 border-gray-300">
                        {
                            self.messages.iter().map(|m| {
                                // Gunakan fallback agar tidak crash jika user belum ada di list
                            let default_avatar = String::from("https://avatars.dicebear.com/api/adventurer-neutral/default.svg");
    ...
    fn update(&mut self, _ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            Msg::HandleMsg(s) => {
                // Gunakan match untuk menangkap error tanpa membuat aplikasi crash
                match serde_json::from_str::<WebSocketMessage>(&s) {
                    Ok(msg) => match msg.message_type {
                        ...
                        MsgTypes::Message => {
                            if let Some(data) = msg.data {
                                // Tangkap error saat parsing isi pesan
                                match serde_json::from_str::<MessageData>(&data) {
                                    ...
                                    Err(e) => {
                                        log::error!("Gagal parse MessageData: {:?}", e);
                                        return false;
                                    }
                                }
                            }
                    ...
                    Err(e) => {
                        log::error!("Pesan WebSocket tidak valid: {:?} dari data: {}", e, s);
                        false
                    }
                }
            }
            Msg::SubmitMessage => {
                ...
                // Ubah menjadi false
                // Agar ini memastikan komponen tidak di-render ulang sebelum
                // mendapat balasan dari server WebSocket.
                false
            }
        }
    }
